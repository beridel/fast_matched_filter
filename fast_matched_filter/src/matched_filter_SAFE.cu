...
__global__ void network_corr(float *templates, float *sum_square_template, int *moveout, float *data,
    ...
    while(t_idx < n_samples_template) {
        ...
        if ((first_sample_block / step + t_idx) < n_corr) data_s[t_idx] = data[data_offset + t_idx];
        t_idx += blockDim.x;
    }
    while(t_idx < (blockDim.x * step + n_samples_template)){
        if ((first_sample_block / step + t_idx) < n_corr) data_s[t_idx] = __ldg(&data[data_offset + t_idx]);
        t_idx += blockDim.x;
    }
