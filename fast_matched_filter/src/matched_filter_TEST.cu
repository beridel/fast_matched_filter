...
__global__ void network_corr(float *templates, float *sum_square_template, int *moveout, float *data,
    ...
    idx = blockIdx.x * blockDim.x + str_offset * n_stations;
    ...
    for (c=0; c < n_components; c++){
      ...
      cc_mat_offset = (first_sample_block / step + threadIdx.x - str_offset) * n_stations;
      //if (cc_mat_offset > streamSize*n_stations) printf("Problem offset %d against streamSize %d \n", cc_mat_offset, streamSize);
      if (cc_mat_offset < streamSize*n_stations){
...
//
//    // 1 block processes one channel to blockDim.x / step different positions in time
//    idx = blockIdx.x * blockDim.x + str_offset * n_stations * n_components;
//    first_sample_block = (idx / (n_stations * n_components)) * step;
//    //s = idx / (n_samples_data * n_components);
//    //c = (idx % (n_samples_data * n_components)) / n_samples_data;
//    s = (idx / n_components) % n_stations;
//    c = idx % n_components;
//
//    // compute offsets for input variables
//    cc_mat_offset = (first_sample_block / step + threadIdx.x - str_offset) * n_stations;
...
__global__ void sum_cc(float *cc_mat, float *cc_sum, float *weights,
        int n_stations, int n_corr, int str_offset, int streamSize) {

    int i, s;

    i = blockIdx.x * blockDim.x + threadIdx.x;
    if ( ((i + str_offset) < n_corr) & (i < streamSize) ) {
        float *cc_mat_offset;
...
void matched_filter(float *templates, float *sum_square_templates, 
    ...
    int nStreams = 10;
    int streamSize = n_corr/nStreams;
    ...
    size_t sizeof_cc_mat = sizeof(float) * streamSize * n_stations; // cc matrix for one template
    size_t sizeof_cc_sum = sizeof(float) * streamSize; // cc sums for one template
...
#pragma omp critical
            ...
            dim3 BS(BLOCKSIZE);
            //dim3 GS(ceilf((streamSize * n_components * n_stations) / (float)BS.x));
            dim3 GS(ceilf((streamSize * n_stations) / (float)BS.x));
            ...
            for (int st = 0; st < nStreams; st++){
                checkCuda( cudaStreamCreate(&streams[st]) );
            }
            ...
            for (int st = 0; st < nStreams; st++){
              int str_offset = st*streamSize;
              // process
              cudaMemsetAsync(cc_mat_d, 0, sizeof_cc_mat); // initiailize cc_mat to 0
              //cudaMemset(cc_mat_d, 0, streamSize * n_stations); // initiailize cc_mat to 0
              network_corr<<<GS, BS, sharedMem, streams[st]>>>(templates_d_t, 
              ...
              cudaMemsetAsync(cc_sum_d, 0, sizeof_cc_sum);
              //cudaMemset(cc_sum_d, 0, streamSize);

              //dim3 GS_sum(ceilf(n_corr_t / (float)BS.x));
              dim3 GS_sum(streamSize/BS.z+1);
              sum_cc<<<GS_sum, BS, 0, streams[st]>>>(cc_mat_d, cc_sum_d, weights_d_t, n_stations, n_corr_t, str_offset, streamSize);
              ...
              cc_sums_t = cc_sums + t_thread * n_corr + str_offset;
              cudaMemcpyAsync(cc_sums_t, cc_sum_d, sizeof_cc_sum, cudaMemcpyDeviceToHost, streams[st]);
            }
