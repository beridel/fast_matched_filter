...
step = 1 # temporal step (in nb of samples) used between each consecutive correlation
SampR = 64. # Sampling rate (nb of samples per second)
Tdata = 24.*60.*60. # Length in seconds of one data trace
...
moveouts = np.zeros((n_templates, ns), dtype=np.int32)
for t in xrange(n_templates):
    moveouts[t,:] = np.int32(np.random.uniform(0., max_moveout, size=ns) * SampR)
# --- randomly pick the templates from the data ---
start_times = np.int32(np.random.uniform(0, Nsamp_data-Nsamp_tp-np.int32(max_moveout*SampR), size=n_templates))
templates = np.zeros((n_templates, ns, nc, Nsamp_tp), dtype=np.float32)
for t in xrange(n_templates):
    for s in xrange(ns):
        for c in xrange(nc):
            idx1 = start_times[t] + moveouts[t,s]
            idx2 = idx1 + Nsamp_tp
            templates[t,s,c,:] = Data[s,c,idx1:idx2]
# --- equal weights for everybody ---
weights = np.ones(ns, dtype=np.float32)
...
weights = np.ones((n_templates, ns), dtype=np.float32)
weights /= ns
...
for i in xrange(len(steps)):
    ...
    for nt in xrange(n_templates):
        ...
        CCs = fmf.matched_filter(templates[:nt+1,:,:,:], weights, moveouts, Data, steps[i], arch='gpu')
        t2 = dt.datetime.now()
